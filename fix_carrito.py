...
idx1 = c.find('window.actualizarModalCarrito = function() {')
idx2 = c.find('window.procesarCheckout = function() {')

if idx1 != -1 and idx2 != -1:
    old_code = c[idx1:idx2]
    new_code = """window.actualizarModalCarrito = function() {
    ...
    if (carritoVentas.length === 0) {
        tbody.innerHTML = '<tr><td colspan="4" class="text-center text-secondary py-4">El carrito est\u00E1 vac\u00EDo</td></tr>';
    ...
    carritoVentas.forEach(item => {
        ...
        tr.innerHTML = 
            ...
            '<td class="align-middle">' +
                ...
                '</div>' +
            '</td>' +
            '<td class="align-middle text-end fw-bold text-success">$' + subt.toFixed(2) + '</td>';
...
"""
    ...
    with io.open('src/main/webapp/includes/scripts.jsp', 'w', encoding='utf-8') as f:
        f.write(c)
    print("Fixed actualizarModalCarrito!")
else:
