...
pattern = re.compile(r'List<Map<String, String>> medsGraf = \(List<Map<String, String>>\) request\.getAttribute\("listaMedicamentos"\);\s*if \(medsGraf != null\) \{\s*int countM = 0;\s*for \(Map<String, String> mg : medsGraf\) \{\s*if \(countM\+\+ >= 5\) break;\s*nomMeds\.add\("\\\"" \+ mg\.get\("nombre"\)\.replace\("\\\"", ""\) \+ "\\\""\);\s*try \{ stkMeds\.add\(Integer\.parseInt\(mg\.get\("stock"\)\)\); \} catch\(Exception ex\) \{ stkMeds\.add\(0\); \}\s*\}\s*\}')

new_code = '''List<Map<String, String>> medsGraf = (List<Map<String, String>>) request.getAttribute("listaMedicamentos");
                if (medsGraf != null) {
