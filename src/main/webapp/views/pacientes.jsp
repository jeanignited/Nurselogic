                                        ...
                                        if(pacientes.isEmpty()) {
                                            ...
                                            for(Map<String, String> p : pacientes) {
                                                ...
                                                if(isAdmin || permPac) {
                                                    ...
                                                    out.print("<button class='btn btn-sm btn-outline-info me-2 mb-1' title='Ficha Médica y Diagnóstico Inteligente' onclick=\"verFichaClinica('" + p.get("cedula") + "')\"><i class='bi bi-eye-fill'></i> Ver Ficha</button>");

                                                    out.print("<button class='btn btn-sm btn-outline-success me-2 mb-1' title='Prescribir Receta Médica' onclick=\"abrirModalReceta('" + p.get("nombres") + " " + p.get("apellidos") + "')\"><i class='bi bi-prescription2'></i> Receta</button>");

                                                    out.print("<button class='btn btn-sm btn-outline-primary me-2 mb-1' title='Editar Paciente' onclick=\"editarPaciente('" + p.get("cedula") + "')\"><i class='bi bi-pencil'></i></button>");

                                                    if(isAdmin) {

                                                        out.print("<button class='btn btn-sm btn-outline-danger' title='Eliminar Paciente' onclick=\"confirmarBorrado('paciente', '" + p.get("cedula") + "')\"><i class='bi bi-trash'></i></button>");
