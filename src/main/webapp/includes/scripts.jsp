...
function abrirModalVerDiagnosticoCama(paciente, btnEl) {
    var diagnostico = btnEl.getAttribute('data-diagnostico') || '';
    var motivo      = btnEl.getAttribute('data-motivo')     || 'No especificado';

    if (!diagnostico && !motivo) {
        Swal.fire({
            title: 'Sin información',
            text: 'No hay diagnóstico registrado para esta cama.',
            icon: 'info',
    ...
    var htmlContent =
        '<div class="text-start">' +
            '<div class="mb-3 p-3 rounded-3" style="background: rgba(59,130,246,0.08); border-left: 4px solid #3b82f6;">' +
                '<div class="small text-secondary fw-semibold mb-1"><i class="bi bi-info-circle-fill me-1 text-primary"></i>Motivo de Internación</div>' +
                '<div class="fw-semibold text-theme" style="white-space: pre-wrap;">' + (motivo || 'No especificado') + '</div>' +
            '</div>' +
            '<div class="p-3 rounded-3" style="background: rgba(16,185,129,0.08); border-left: 4px solid #10b981;">' +
                '<div class="small text-secondary fw-semibold mb-1"><i class="bi bi-journal-text me-1 text-success"></i>Diagnóstico de Ingreso</div>' +
                '<div class="text-theme" style="white-space: pre-wrap;">' + (diagnostico || 'No especificado') + '</div>' +
            '</div>' +
        '</div>';

    Swal.fire({
        title: '<i class="bi bi-file-medical-fill me-2 text-info"></i>' + paciente,
        html: htmlContent,
        icon: false,
