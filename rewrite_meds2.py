...
def repl_th(m):
    return '''<% if(canSellStock) { out.print("<th class='text-center'><i class='bi bi-cart-plus me-1'></i></th>"); } %>'''
c = re.sub(r'<% if\(canManageStock \|\| canSellStock\) \{ out\.print\("<th>Ajuste R[^<]*</th\>"\); \} %>', repl_th, c)

old_td = r'if \(canManageStock \|\| canSellStock\) \{.*?out\.print\("</td>"\);\s*\}'
def repl_td(m):
    return '''if (canSellStock) {
...
c = re.sub(old_td, repl_td, c, flags=re.DOTALL)
...
c = re.sub(search_bar, repl_search, c, flags=re.DOTALL)
