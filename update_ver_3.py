...
m1 = re.search(r'function abrirModalVerDiagnostico\(paciente, cedula, btnEl\).*?m\.show\(\);\s*\}\s*\}', c, re.DOTALL)
if not m1:
    m1 = re.search(r'function abrirModalVerDiagnostico\(paciente, btnEl\).*?m\.show\(\);\s*\}\s*\}', c, re.DOTALL)

new_code_1 = r'''function abrirModalVerDiagnostico(paciente, cedula, btnEl) {
...
c = c.replace(m1.group(0), new_code_1)

with io.open('src/main/webapp/includes/scripts.jsp', 'w', encoding='utf-8') as f:
    f.write(c)
print("Updated abrirModalVerDiagnostico")
